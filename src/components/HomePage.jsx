    ...
    <>
      ...
      <section className="container mx-auto px-4 py-16 text-center">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-bold mb-6" style={{ color: currentTheme.primary }}>Bilimi Keşfet, Eğlenerek Öğren</h2>
          <p className="text-lg mb-8 text-gray-600">Çocuklar ve yetişkinler için özenle seçilmiş bilim ürünleri. Oyunla öğrenmenin, merakla büyümenin adresi.</p>
          <div className="flex gap-4 justify-center flex-wrap"><button onClick={() => document.getElementById('products').scrollIntoView({ behavior: 'smooth' })} className="px-8 py-3 rounded-full text-white font-semibold transition-all transform hover:scale-105" style={{ backgroundColor: currentTheme.primary }}>Ürünleri Keşfet</button><button onClick={() => document.getElementById('about').scrollIntoView({ behavior: 'smooth' })} className="px-8 py-3 rounded-full border-2 font-semibold transition-all hover:bg-gray-50" style={{ borderColor: currentTheme.primary, color: currentTheme.primary }}>Hikayemiz</button></div>
        </div>
      ...
      <section id="products" className="container mx-auto px-4 py-16">
        ...
        <div className="max-w-xl mx-auto mb-8">
          <div className="relative">
            ...
            <input id="product-search" type="text" placeholder="Ürün ara..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 rounded-full border border-gray-200 focus:outline-none focus:ring-2 transition-all" style={{ focusRingColor: currentTheme.accent }}/>
          </div>
        </div>
        ...
        {products.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{products.map(product => (<div key={product.id} className={`${currentTheme.cardBg} rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer`} onClick={() => handleProductClick(product)}><div className="relative h-48 bg-gray-100"><img src={product.image} alt={product.name} className="w-full h-full object-cover" loading="lazy"/>{product.badge && <span className="absolute top-2 left-2 px-3 py-1 text-xs font-bold text-white rounded-full" style={{ backgroundColor: currentTheme.accent }}>{product.badge}</span>}<button onClick={(e) => { e.stopPropagation(); toggleLike(product.id); }} className="absolute top-2 right-2 p-2 bg-white/80 rounded-full hover:bg-white transition-colors"><Heart className={`w-5 h-5 ${likedProducts.has(product.id) ? 'fill-red-500 text-red-500' : 'text-gray-600'}`}/></button></div><div className="p-4"><h3 className="font-bold text-lg mb-2" style={{ color: currentTheme.text.replace('text-', '') }}>{product.name}</h3><div className="flex flex-wrap gap-2 mb-3"><span className="text-xs px-2 py-1 bg-gray-100 rounded-full">{product.category}</span><span className="text-xs px-2 py-1 bg-gray-100 rounded-full">{product.ageGroup} yaş</span><span className="text-xs px-2 py-1 text-white rounded-full" style={{ backgroundColor: themes[product.science]?.primary || currentTheme.primary }}>{product.science}</span></div><div className="flex items-center gap-1 mb-2"><Star className="w-4 h-4 fill-yellow-400 text-yellow-400" /><span className="text-sm font-medium">{product.rating}</span><span className="text-sm text-gray-500">({product.reviews})</span></div><p className="text-sm text-gray-600 mb-4 line-clamp-2">{product.description}</p><div className="flex items-center gap-2 mb-4"><span className="text-2xl font-bold" style={{ color: currentTheme.primary }}>₺{product.price}</span>{product.originalPrice && <span className="text-sm text-gray-400 line-through">₺{product.originalPrice}</span>}</div><div className="flex flex-col gap-2"><button onClick={(e) => { e.stopPropagation(); addToCart(product); }} className="w-full py-2 px-4 rounded-lg text-white font-medium transition-all hover:opacity-90" style={{ backgroundColor: currentTheme.primary }}><ShoppingCart className="w-5 h-5 inline-block mr-1" />Sepete Ekle</button><button onClick={(e) => { e.stopPropagation(); buyNow(product); }} className="w-full py-2 px-4 rounded-lg border transition-colors hover:bg-gray-50" style={{ borderColor: currentTheme.primary, color: currentTheme.primary }}>Hemen Satın Al</button></div></div></div>))}</div>) : (<div className="text-center py-12"><p className="text-gray-500 mb-4">Aradığınız kriterlere uygun ürün bulunamadı.</p><button onClick={clearFilters} className="px-6 py-2 rounded-full text-white font-medium" style={{ backgroundColor: currentTheme.primary }}>Filtreleri Temizle</button></div>)}
      </section>
      ...
      <section id="about" className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left"><div className="p-6 bg-white rounded-lg shadow-md flex flex-col items-center text-center transition-all hover:scale-105 duration-300"><Award className="w-12 h-12 mb-4" style={{ color: currentTheme.primary }} /><h3 className="font-bold text-xl mb-2">Kalite Garantisi</h3><p className="text-gray-600 text-sm">Ürünlerimizin tamamı uluslararası standartlara uygun, dayanıklı ve güvenlidir.</p></div><div className="p-6 bg-white rounded-lg shadow-md flex flex-col items-center text-center transition-all hover:scale-105 duration-300"><Package className="w-12 h-12 mb-4" style={{ color: currentTheme.primary }} /><h3 className="font-bold text-xl mb-2">Hızlı Kargo</h3><p className="text-gray-600 text-sm">Siparişleriniz, en kısa sürede ve özenle paketlenerek kapınıza ulaştırılır.</p></div><div className="p-6 bg-white rounded-lg shadow-md flex flex-col items-center text-center transition-all hover:scale-105 duration-300"><Shield className="w-12 h-12 mb-4" style={{ color: currentTheme.primary }} /><h3 className="font-bold text-xl mb-2">Güvenli Alışveriş</h3><p className="text-gray-600 text-sm">Tüm ödeme ve kişisel bilgileriniz en üst düzey güvenlik önlemleriyle korunur.</p></div></div>
        </div>
      </section>
      ...
      <section id="faq" className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-4">{faqData.map((faq, index) => (<div key={index} className="border border-gray-200 rounded-lg p-4 cursor-pointer transition-all duration-300 hover:shadow-sm" onClick={() => setActiveFaq(activeFaq === index ? null : index)}><div className="flex justify-between items-center"><h3 className="font-semibold text-lg" style={{ color: currentTheme.text.replace('text-', '') }}>{faq.question}</h3>{activeFaq === index ? <ChevronUp className="w-6 h-6 text-gray-500" /> : <ChevronDown className="w-6 h-6 text-gray-500" />}</div>{activeFaq === index && <p className="mt-4 text-gray-600">{faq.answer}</p>}</div>))}</div>
        </div>
      </section>
      ...
      <footer className="bg-slate-900 text-white py-12">
        ...
        <div className="border-t border-gray-700 mt-8 pt-8 text-center text-gray-500"><p>&copy; 2025 Atomu Parçalayan Bebek. Tüm hakları saklıdır.</p></div>
      </footer>
    </>
  );
}