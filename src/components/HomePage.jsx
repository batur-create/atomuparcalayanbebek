      ...
      <section id="products" className="container mx-auto px-4 py-16">
        ...
        <div className="mb-8 space-y-4">
          <div className="flex flex-wrap gap-2 justify-center">
            ...
            {['oyuncak', 'deney', 'eğitici', 'dekoratif'].map(cat => (<button key={cat} onClick={() => handleFilterChange('category', cat)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${activeFilters.category === cat ? 'text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`} style={{backgroundColor: activeFilters.category === cat ? currentTheme.primary : undefined}}>{cat.charAt(0).toUpperCase() + cat.slice(1)}</button>))}
          </div>
          <div className="flex flex-wrap gap-2 justify-center">
            <span className="text-sm font-semibold text-gray-600 mr-2 self-center">Yaş:</span>
            {['3-6', '7-12', '13+'].map(age => (<button key={age} onClick={() => handleFilterChange('ageGroup', age)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${activeFilters.ageGroup === age ? 'text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`} style={{backgroundColor: activeFilters.ageGroup === age ? currentTheme.primary : undefined}}>{age} Yaş</button>))}
          </div>
          <div className="flex flex-wrap gap-2 justify-center">
            <span className="text-sm font-semibold text-gray-600 mr-2 self-center">Bilim Dalı:</span>
            {['fizik', 'kimya', 'biyoloji', 'astronomi'].map(sci => (<button key={sci} onClick={() => handleFilterChange('science', sci)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all flex items-center gap-2 ${activeFilters.science === sci ? 'text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`} style={{backgroundColor: activeFilters.science === sci ? themes[sci].primary : undefined}}>{sci === 'fizik' && <Atom className="w-4 h-4" />}{sci === 'kimya' && <TestTube className="w-4 h-4" />}{sci === 'biyoloji' && <Microscope className="w-4 h-4" />}{sci === 'astronomi' && <Rocket className="w-4 h-4" />}{sci.charAt(0).toUpperCase() + sci.slice(1)}</button>))}
          </div>
              ...
              <div key={product.id} className={`${currentTheme.cardBg} rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer`} onClick={() => handleProductClick(product)}>
                ...
                <div className="p-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <button onClick={(e) => { e.stopPropagation(); addToCart(product); }} className="w-full py-2 px-4 rounded-lg text-white font-medium transition-all hover:opacity-90" style={{ backgroundColor: currentTheme.primary }}>
                      <ShoppingCart className="w-5 h-5 inline-block mr-1" />
                      Sepete Ekle
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); buyNow(product); }} className="w-full py-2 px-4 rounded-lg border transition-colors hover:bg-gray-50" style={{ borderColor: currentTheme.primary, color: currentTheme.primary }}>
                      Hemen Satın Al
                    </button>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div>
                    ...
                    <form onSubmit={(e) => e.preventDefault()} className="flex">
                        ...
                        <button type="submit" className="px-4 py-2 bg-purple-600 text-white rounded-r-lg font-bold hover:bg-purple-700 transition-colors">
                          Abone Ol
                        </button>
                    </form>
                </div>
